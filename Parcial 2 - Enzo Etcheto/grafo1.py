...
class maravilla:
    ...
    def __str__(self):
        return f'{self.nombre}-{self.lugar}-{self.tipo}'
...
class Grafo():
    ...
    def barrido_mayor(self):
        ...
        for value in self.__elements:
            ...
            for i in range(aristas.size()):
                arista = aristas.get_element_by_index(i)
                if arista.peso > max_peso:
                    max_peso = arista.peso
                    max_arista = arista
                    vertice_ori = value[0]
                    vertice_des = arista.vertice

        if max_arista is not None:
            print(f'Arista de mayor peso: {vertice_ori} - {vertice_des} - Peso: {max_arista.peso}')
    ...
    def kruskal(self):
        ...
        control=buscar_en_bosque(bosque,'Yoda')
